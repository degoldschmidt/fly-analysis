...
folder = "/Volumes/Elements/raw_data_flies/0805/"
filename="VidSave_0805_15-18-11.avi" 
profolder = "../tmp/vid/"

if not os.path.isfile(profolder + "arena_" + filename):
    os.system("ffmpeg -i " + folder + filename + " -t 00:00:01" + profolder + "arena_" + filename ) ## maybe in fly logger

###### Game defs
pg.init()
if args.filename == None:
    cap = cv2.VideoCapture(profolder + "arena_" +  filename)
    ret, img = cap.read()
    cv2.imwrite('../tmp/arena.bmp', img)
    cap.release()
    background = pg.image.load('../tmp/arena.bmp')
else:
